...
#    name = OUT+"_"+y+"_"+m+"_GribIndexPoints_"+forthenameoffile+".csv";
        ...
        while 1:
         ...
         for x in indexarray:
          if i==int(x):
            ...
            #if lat >=54.0651 and lat<=54.1051 and lon >=359.7829 and lon <=359.8229 and codes_get(iterid, 'dataDate')=='20190101': #EA1
            #if lat >=54.0651 and lat<=54.1051 and lon >=359.7829 and lon <=359.8229 and codes_get(iterid, 'validityDate')==20190101 and codes_get(iterid, 'validityTime')==100: #EA1
            #if lat >=54.0651 and lat<=54.1051 and lon >=359.7829 and lon <=359.8229 and codes_get(iterid, 'validityTime')==100: #EA1
            #if codes_get(iterid, 'validityDate')==20190101 and codes_get(iterid, 'validityTime')==100: #EA1
...
#indexler icin olanlar
             #sys.stdout.write("%d,%.6f,%.6f,%.6f,%s,%s,%s,%s,%s,%s,%s\n" % (i, lat, (lon-360), value, codes_get(iterid, 'dataDate'), codes_get(iterid, 'dataTime'), codes_get(iterid, 'validityDate'), codes_get(iterid, 'validityTime'), codes_get(iterid, 'name'), codes_get(iterid, 'shortName'),codes_get(iterid, 'units')))
             #sys.stdout.write("%d,%.6f,%.6f\n" % (i, lat, lon))
             #f2.write("%d,%.6f,%.6f,%.6f,%s,%s,%s,%s,%s,%s,%s\n" % (i, lat, (lon-360), value, codes_get(iterid, 'dataDate'), codes_get(iterid, 'dataTime'), codes_get(iterid, 'validityDate'), codes_get(iterid, 'validityTime'), codes_get(iterid, 'name'), codes_get(iterid, 'shortName'),codes_get(iterid, 'units')))
             #if lon<250:
              #f2.write("<marker %d id=\"GribIndex-%d\" lng=\"%.6f\" lat=\"%.6f\" />\n" % (value, i, lon, lat))
             #elif lon>250:
              #f2.write("<marker %d id=\"GribIndex-%d\" lng=\"%.6f\" lat=\"%.6f\" />\n" % (value, i, (lon-360), lat))
...
#    Process(target = example,args=('/var/www/html/mera/map/backup/thread/Press/MERA_PRODYEAR_'+year+'_'+smonth+'_1_103_0_0_FC3hr','Pressure','true',smonth,year,yar)).start()
    Process(target = example,args=('/var/www/html/mera/map/backup/thread/Soil/MERA_PRODYEAR_'+year+'_'+smonth+'_86_105_801_0_FC3hr','Soil','true',smonth,year,yar)).start()
